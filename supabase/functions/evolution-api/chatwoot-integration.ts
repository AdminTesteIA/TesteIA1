import { supabase } from './supabase-client.ts';const CHATWOOT_CONFIG = {
...
export async function createChatwootUser(agentData: any): Promise<any> {
  ...
  console.log(' [CHATWOOT] User creation response body:', responseText);  if (!response.ok) {
...
export async function createChatwootAccount(agentData: any): Promise<number> {
 ...
 CHATWOOT_CONFIG.PLATFORM_TOKEN.substring(0, 10)
  ...
  const requestBody = {
    ...
    locale: 'pt_BR',
  };  console.log(' [CHATWOOT] Request Body:', JSON.stringify(requestBody, null, 2));  const response = await fetch(${CHATWOOT_CONFIG.URL}/platform/api/v1/accounts, {
    method: 'POST',
    headers: {
      'api_access_token': CHATWOOT_CONFIG.PLATFORM_TOKEN,
      'Content-Type': 'application/json',
  ...
  console.log(' [CHATWOOT] Response Status Text:', response.statusText);  const responseHeaders: Record<string, string> = {};
  response.headers.forEach((value, key) => {
    responseHeaders[key] = value;
  });
  console.log(' [CHATWOOT] Response Headers:', JSON.stringify(responseHeaders, null, 2));  const responseText = await response.text();
  console.log(' [CHATWOOT] Response Body (raw):', responseText);  if (!response.ok) {
...
export async function createChatwootAgent(
  ...
  const requestBody = {
    user_id: userId,
    role: 'administrator',
  };  console.log(' [CHATWOOT] Creating account_user with body:', JSON.stringify(requestBody, null, 2));  const response = await fetch(
  ...
  console.log(' [CHATWOOT] Agent creation response status text:', response.statusText);  const responseHeaders: Record<string, string> = {};
  response.headers.forEach((value, key) => {
    responseHeaders[key] = value;
  });
  console.log(' [CHATWOOT] Agent creation response headers:', JSON.stringify(responseHeaders, null, 2));  const responseText = await response.text();
  console.log(' [CHATWOOT] Agent creation response body:', responseText);  if (!response.ok) {
...
export async function getOrCreateChatwootSetup(
  ...
  };  console.log(' [CHATWOOT] Enriched Agent Data:', JSON.stringify(enrichedAgentData, null, 2));  // 3) Verificar se já existe configuração salva (accountId + agentToken)
  ...
  const { data: existingWhatsapp } = await supabase
    ...
    .eq('agent_id', agentId)
    .single();  console.log(' [CHATWOOT] Existing WhatsApp data:', existingWhatsapp);  if (
  ...
  const agentToken = await createChatwootAgent(accountId, enrichedAgentData);  console.log(' [CHATWOOT] Setup completed successfully');
  console.log(' [CHATWOOT] Final Account ID:', accountId);
  console.log(' [CHATWOOT] Final Agent Token (first 10 chars):', agentToken.substring(0, 10));  // 5) Salvar no Supabase (sem inboxId)
